...
class MyHostApduService : HostApduService() {
    ...
    override fun processCommandApdu(commandApdu: ByteArray?, extras: Bundle?): ByteArray {
        commandApdu?.let { apdu ->
            ...
            return when (command.kind) {
                ...
                CommandKind.MERCHANT_PAYMENT_AUTHORIZATION -> {
                    handleMerchantPaymentAuthorization(command.payload)
                }
            }
        }
    ...
    private fun decodeCommand(apdu: ByteArray): DecodedCommand? {
        return try {
            ...
            when (decodeCommandKind(json)) {
                ...
                CommandKind.MERCHANT_PAYMENT_AUTHORIZATION -> {
                    DecodedCommand(CommandKind.MERCHANT_PAYMENT_AUTHORIZATION, json)
                }
                null -> null
            }
        } catch (error: JSONException) {
            Log.d(TAG, "Not a JSON command: ${error.message}")
            null
        }
    }
    ...
    private fun isSelectAidCommand(apdu: ByteArray): Boolean {
        ...
        for (index in AID_BYTES.indices) {
            if (apdu[aidStart + index] != AID_BYTES[index]) return false
        }
