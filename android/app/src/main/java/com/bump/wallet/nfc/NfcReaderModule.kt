...
class NfcReaderModule(
    ...
    override fun onTagDiscovered(tag: Tag?) {
        ...
        try {
            ...
            closeQuietly(iso)
            clearActiveSession()
            emitReaderError("Payment discovery failed: ${error.message}")
        }
    }
    ...
    private fun sendIntent(iso: IsoDep, payload: String, promise: Promise) {
        try {
            ...
            promise.reject("INTENT_FAILED", "Failed to send payment intent: ${error.message}")
        } finally {
            clearActiveSession()
        }
    }
    ...
    private fun sendAuthorization(iso: IsoDep, payload: String, promise: Promise) {
        try {
            ...
            promise.reject("AUTH_FAILED", "Failed to send authorization: ${error.message}")
        } finally {
            clearActiveSession()
        }
    }
    ...
    fun startReader(promise: Promise) {
        try {
            ...
            promise.reject("READER_ERROR", "Failed to enable reader mode: ${error.message}")
        }
    }
    ...
    fun stopReader(promise: Promise) {
        try {
            ...
            promise.reject("STOP_ERROR", "Failed to disable reader mode: ${error.message}")
        }
    }
